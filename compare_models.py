# compare_models.py
# head-to-head comparison of xgboost vs lstm + calibration layer.
#
...
def load_xgboost_preds(target, df):
    ...
    X_val, y_val   = df[features].iloc[i1:i2], df[target].iloc[i1:i2]
    X_test, y_test = df[features].iloc[i2:],   df[target].iloc[i2:]
...
def load_lstm_preds(target, df):
    """load cached lstm predictions from csv -- no pytorch needed."""
...
def run_target(target, df_full):
    ...
    lstm_p = load_lstm_preds(target, df_full)
    ...
    return {"xgboost": m_xgb, "lstm": m_lstm, "persistence": m_base,
            "xgb_sigma": xgb_sigma, "lstm_sigma": lstm_sigma}
