...
# target columns and their renamed equivalents
KEEP_COLS = {
    "STATION":                  "station_id",
...
def load_psv(path: str) -> pd.DataFrame:
    """load one psv file, keep only useful columns, return a tidy dataframe."""
...
def resample_hourly(df: pd.DataFrame) -> pd.DataFrame:
    """floor to the hour and aggregate: mean for most cols, sum for precip."""
    df = df.copy()
...
def main():
    ...
    print(f"\ncombined + deduped: {len(combined):,} hourly rows")
